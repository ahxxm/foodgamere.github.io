 ...
 * 厨神大赛优化器（性能优化版）
 *
 * 本文件是 js/competition.optimizer.simple.js 的优化分支副本：
 * 复用同一套搜索算法，但对无限分模式的性能热点做了优化
 * （食材平面索引、粗排+精排分层、apply跳过、缓存键前置等）。
 * 参见 tmp/go-optimizer/ 的设计思路。
 *
 * 注意：这是 simple.js 的 fork；上游对 simple.js 的修复需手动同步到本文件。
 */
...
var DefaultCompetitionOptimizer = (function() {
    ...
    var _calcScoreCache = new Map();
    var _calcScoreCacheMaxSize = 8000;
    var _materialsMap = null;        // {materialId: qty}，由_rule.materials构建（首个重复项生效，与getRecipeQuantity的break语义一致）
    var _chefTemplateCache = null;   // chefId -> 预处理厨师模板（maxDisk/useAmber已应用）
    ...
    function init(gameData) {
        ...
        _recipeMap = {};
        _menus = [];
        _chefTemplateCache = Object.create(null);

        // 食材平面索引（首个重复项生效，匹配getRecipeQuantity的break语义）
        _materialsMap = Object.create(null);
        if (_rule.materials) {
            for (var mti in _rule.materials) {
                var mt = _rule.materials[mti];
                if (mt && mt.materialId !== undefined && _materialsMap[mt.materialId] === undefined) {
                    _materialsMap[mt.materialId] = mt.quantity;
                }
            }
        }
    ...
    function _cloneSimState(state) {
        ...
        for (var ci = 0; ci < state[0].length; ci++) {
            ...
            clone[0].push({
                chefId: s.chefId,
                chefObj: s.chefObj ? _deepClone(s.chefObj) : null,
                equipObj: s.equipObj ? _deepClone(s.equipObj) : {},
                condiment: s.condiment ? _deepClone(s.condiment) : {},
                recipes: [
    ...
    function _simSetChef(chefIndex, chefId) {
        ...
        var srcChef = _chefMap[chefId] || null;
        if (!srcChef && _rule.chefs) {
        ...
        // 模板缓存：maxDisk/useAmber调整是chefId的确定函数，只预处理一次
        var template = _chefTemplateCache[chefId];
        if (!template) {
            template = _deepClone(srcChef);
            if (_cachedConfig.maxDisk) {
                template.disk.level = template.disk.maxLevel;
            }
            if (!_cachedConfig.useAmber) {
                for (var ai = 0; ai < template.disk.ambers.length; ai++) {
                    template.disk.ambers[ai].data = null;
                }
            }
            _chefTemplateCache[chefId] = template;
        }

        slot.chefId = chefId;
        slot.chefObj = _deepClone(template);
    ...
    function _calcGlobalRemainMaterials(excludeChef, excludeRecipe) {
        ...
        var remain = Object.assign(Object.create(null), _materialsAll);
            ...
            for (var ri = 0; ri < 3; ri++) {
                ...
                if (rec.data && rec.quantity > 0) {
                    for (var m = 0; m < rec.data.materials.length; m++) {
                        ...
                        var qty = _calMaterialReduceFast(_simState[0][ci].chefObj, mat.material, mat.quantity);
    ...
    function _calcRemainMaterials(excludeChef, excludeRecipe) {
        // 返回{materialId: qty}平面表，配合_fastGetRecipeQuantity使用
        var remainMaterials = Object.assign(Object.create(null), _materialsMap);
        ...
        for (var ci = 0; ci < ruleState.length; ci++) {
            for (var ri = 0; ri < 3; ri++) {
                ...
                if (rec.data && rec.quantity > 0) {
                    var chefObj = ruleState[ci].chefObj;
                    for (var m = 0; m < rec.data.materials.length; m++) {
                        var mat = rec.data.materials[m];
                        if (remainMaterials[mat.material] === undefined) continue;
                        if (!Number.isInteger(parseInt(remainMaterials[mat.material]))) continue;
                        var per = _calMaterialReduceFast(chefObj, mat.material, mat.quantity);
                        remainMaterials[mat.material] -= per * rec.quantity;
                    }
                }
            }
        }
                        ...
                        if (rec.data && rec.quantity > 0) {
                            ...
                            for (var m = 0; m < rec.data.materials.length; m++) {
                                if (rec.data.materials[m].material.toString() === overflowMat.toString()) {
                                    perUnit = _calMaterialReduceFast(_simState[0][ci].chefObj, rec.data.materials[m].material, rec.data.materials[m].quantity);
                                    break;
                                }
                            }
                            if (perUnit > 0) {
    ...
    function _simSetRecipe(chefIndex, recipeIndex, recipeId) {
        ...
        var qty = _fastGetRecipeQuantity(recipeData, remainMaterials, slot.chefObj);

        // 全局食材限制
        if (_rule.MaterialsLimit && _materialsAll) {
            var globalRemain = _calcGlobalRemainMaterials(chefIndex, recipeIndex);
            var materialLimit = _fastMaterialLimit(globalRemain, recipeData, slot.chefObj);
    ...
    function _applyRecipeCandidate(chefIndex, recipeIndex, candidate) {
        ...
        var savedEquip = slot.equipObj ? _deepClone(slot.equipObj) : {};
    ...
    var _objIdMap = new WeakMap();
    var _objIdCounter = 0;
    var _appliedKey = '';
    var _appliedByFull = false;
    function _oid(o) {
        if (!o) return '0';
        var id = _objIdMap.get(o);
        if (id === undefined) { id = ++_objIdCounter; _objIdMap.set(o, id); }
        return id;
    }
    function _buildApplyKey(ruleState) {
        var s = '';
        for (var ci = 0; ci < ruleState.length; ci++) {
            s += _oid(ruleState[ci].chefObj) + ':' + _oid(ruleState[ci].equipObj) + ':' + _oid(ruleState[ci].condiment) + '|';
        }
        return s;
    }
    ...
    function _applyChefData() {
        var ruleState = _simState[0];
        var key = _buildApplyKey(ruleState);
        if (_appliedByFull && key === _appliedKey) return;
    ...
    function _applyChefDataSingle(chefIndex) {
        ...
        // 单槽apply使用缓存的partialAdds，可能与其他槽的最新状态不一致；
        // 标记非全量，迫使下次_applyChefData重算partialAdds。
        _appliedKey = _buildApplyKey(ruleState);
        _appliedByFull = false;
    }
    ...
    function _refreshRecipeQuantitiesForChef(chefIndex) {
        ...
        for (var ri = 0; ri < 3; ri++) {
            ...
            var newMax = _fastGetRecipeQuantity(rec.data, _materialsMap, slot.chefObj);
            if (_rule.MaterialsLimit && _materialsAll) {
                var matMax = _fastMaterialLimit(_materialsAll, rec.data, slot.chefObj);
    ...
    function _setCalcScoreCache(key, score) {
        if (_calcScoreCache.has(key)) {
            _calcScoreCache.set(key, score);
            return;
        }
        if (_calcScoreCache.size >= _calcScoreCacheMaxSize) {
            _calcScoreCache.delete(_calcScoreCache.keys().next().value);
        }
        _calcScoreCache.set(key, score);
    }
    ...
    function _getCalcScoreCache(key) {
        var v = _calcScoreCache.get(key);
        return v === undefined ? null : v;
    }


    // calMaterialReduce的快速版：按厨师缓存每种食材的{abs,percent}加成。
    // materialEffects由setDataForChef整体替换，引用变化时缓存自动失效。
    function _calMaterialReduceFast(chefObj, matId, qty) {
        var fx = chefObj && chefObj.materialEffects;
        if (!fx || fx.length === 0) return qty;
        var cache = chefObj._mrCache;
        if (!cache || cache.fx !== fx) {
            cache = chefObj._mrCache = { fx: fx, m: Object.create(null) };
        }
        var add = cache.m[matId];
        if (add === undefined) {
            var abs = 0, percent = 0;
            for (var i = 0; i < chefObj.materialEffects.length; i++) {
                var eff = chefObj.materialEffects[i];
                if (eff.conditionType === 'MaterialReduce' && eff.conditionValueList.indexOf(matId) >= 0) {
                    if (eff.cal === 'Abs') abs = +(abs + eff.value).toFixed(2);
                    else if (eff.cal === 'Percent') percent = +(percent + eff.value).toFixed(2);
                }
            }
            add = { abs: abs, percent: percent };
            cache.m[matId] = add;
        }
        var t = Math.ceil(+((qty - add.abs) * (1 - add.percent / 100)).toFixed(2));
        return t > 1 ? t : 1;
    }


    // getRecipeQuantity的快速版：remainMap为{materialId: qty}平面表，O(菜谱食材数)。
    function _fastGetRecipeQuantity(recipeData, remainMap, chefObj) {
        var limit = 1;
        if (!_rule.DisableMultiCookbook) {
            limit = recipeData.limitVal;
            if (chefObj && chefObj.maxLimitEffect) {
                for (var i = 0; i < chefObj.maxLimitEffect.length; i++) {
                    if (chefObj.maxLimitEffect[i].rarity == recipeData.rarity) {
                        limit += chefObj.maxLimitEffect[i].value;
                    }
                }
            }
        }
        for (var m = 0; m < recipeData.materials.length; m++) {
            ...
            var avail = remainMap[mat.material];
            if (avail === undefined) return 0;
            if (Number.isInteger(parseInt(avail))) {
                var per = _calMaterialReduceFast(chefObj, mat.material, mat.quantity);
                var can = Math.floor(avail / per);
                if (can < limit) limit = can;
            }
        }
    ...
    // calculateMaterialLimit的快速版（语义一致，食材减免走缓存）
    function _fastMaterialLimit(remainMap, recipeData, chefObj) {
        if (!remainMap || !_materialsAll) return 500;
        var limit = 500;
        for (var m = 0; m < recipeData.materials.length; m++) {
            var mat = recipeData.materials[m];
            var per = _calMaterialReduceFast(chefObj, mat.material, mat.quantity);
            var can = Math.floor((remainMap[mat.material] || 0) / per);
            if (can < limit) limit = can;
        }
        return limit;
    }
    ...
    function _calcScore() {
        ...
        // 缓存键不含rec.max：先查缓存，命中时跳过_applyChefData与精算。
        // 缓存键覆盖全部槽位的厨师/厨具/遗玉/菜谱/份数，键相同则计算结果与max刷新结果相同。
        var cacheKey = _buildCalcScoreCacheKey(ruleState);
        var cachedScore = _getCalcScoreCache(cacheKey);
        if (cachedScore !== null) {
            _refreshRecipeMaxOnly(ruleState, hasMaterialsLimit, disableMultiCookbook, stateLen);
            return cachedScore;
        }
    ...
    // 重算每道菜的份数上限max并钳位quantity（用全量食材池，不用剩余量）
    function _refreshRecipeMaxOnly(ruleState, hasMaterialsLimit, disableMultiCookbook, stateLen) {
        for (var ci = 0; ci < stateLen; ci++) {
            ...
            for (var reci = 0; reci < 3; reci++) {
                ...
                var recData = rec.data;
                if (recData) {
                    var recipeMax = _fastGetRecipeQuantity(recData, _materialsMap, chefObj);
                    if (hasMaterialsLimit) {
                        var matLimit = _fastMaterialLimit(_materialsAll, recData, chefObj);
    ...
    function _fastGetChefRanking(chefIndex, fastMode) {
        ...
        if (!hasRecipe) {
            ...
            for (var i = 0; i < _rule.chefs.length; i++) {
                ...
                var skillOk = true;
                var tempChef = _deepClone(chefObj);
                for (var reci = 0; reci < 3; reci++) {
                    if (ruleState[chefIndex].recipes[reci].data) {
    ...
    function _fastGetRecipeRankingBase(chefIndex, recipeIndex, topK, fastMode, preciseEval) {
        var ruleState = _simState[0];
        ...
        // ===== Phase1: 粗排（与Go版一致：价格×份数，RE菜谱加成加权） =====
        var phase1 = [];
        var recipeEffect = _rule.RecipeEffect || null;
        ...
        for (var i = 0; i < _menus.length; i++) {
            ...
            var qty = _fastGetRecipeQuantity(rd, preRemainMaterials, chefObj);
            if (preGlobalRemain) {
                var matLimit = _fastMaterialLimit(preGlobalRemain, rd, chefObj);
            ...
            var reBonus = 0;
            if (recipeEffect) {
                if (recipeEffect[rd.recipeId] != null) {
                    reBonus = recipeEffect[rd.recipeId];
                } else if (recipeEffect[Number(rd.recipeId)] != null) {
                    reBonus = recipeEffect[Number(rd.recipeId)];
                }
            }

            // 神品加成粗估（阈值与getRankInfo一致）
            var rankBonus = 0;
            if (!_rule.DisableCookbookRank && chefObj && chefObj.chefId) {
                var ratio = Number.MAX_VALUE;
                if (rd.stirfry > 0) ratio = Math.min(ratio, chefObj.stirfryVal / rd.stirfry);
                if (rd.boil > 0) ratio = Math.min(ratio, chefObj.boilVal / rd.boil);
                if (rd.knife > 0) ratio = Math.min(ratio, chefObj.knifeVal / rd.knife);
                if (rd.fry > 0) ratio = Math.min(ratio, chefObj.fryVal / rd.fry);
                if (rd.bake > 0) ratio = Math.min(ratio, chefObj.bakeVal / rd.bake);
                if (rd.steam > 0) ratio = Math.min(ratio, chefObj.steamVal / rd.steam);
                if (ratio !== Number.MAX_VALUE) {
                    rankBonus = ratio >= 5 ? 100 : ratio >= 4 ? 50 : ratio >= 3 ? 30 : ratio >= 2 ? 10 : 0;
                }
            }

            var est = (rd.price || 0) * (1 + rankBonus / 100) * (1 + (rd.addition || 0) / 100) * qty * (1 + reBonus / 100);
            // 对RE菜谱：用单份分数×放大系数来排名，确保高RE加成的菜谱不会因为份数少而被过滤
            var estWithBonus = est;
            if (reBonus > 0 && qty > 0) {
                var virtualTotal = (est / qty) * 30;
        ...
        // Phase2: 条件型光环精排。粗排不含光环效应，放大Phase2以覆盖光环偏好菜谱（getRecipeResult比_calcScore便宜）
        var phase2Size = needPhase2 ? Math.max(topK || 5, _simpleMode ? 14 : 8) : 0;
        ...
        // Phase3: 精确分数。Phase2已用getRecipeResult筛选，Phase3只需精算topK个候选。
        var candidates = phase2.length > 0 ? phase2 : phase1;
        var phase3Limit = Math.min(preciseEval || Math.max(topK || 5, _simpleMode ? 6 : 8), candidates.length);
        var results = [];
    ...
    function _getAutoPoolEquipAwareRecipeRanking(chefIndex, recipeIndex, topK, fastMode) {
        ...
        for (var i = 0; i < _menus.length; i++) {
            ...
            var bareQty = _fastGetRecipeQuantity(rd, preRemainMaterials, bareChef);
            if (preGlobalRemain) {
                bareQty = Math.min(bareQty, _fastMaterialLimit(preGlobalRemain, rd, bareChef));
            }
        ...
        for (var ci2 = 0; ci2 < candidates.length; ci2++) {
            ...
            for (var ei = 0; ei < autoPoolEquips.length; ei++) {
                ...
                var eqQty = _fastGetRecipeQuantity(recipeData, preRemainMaterials, equipChef);
                if (preGlobalRemain) {
                    eqQty = Math.min(eqQty, _fastMaterialLimit(preGlobalRemain, recipeData, equipChef));
                }
        ...
        for (var pi = 0; pi < roughPairs.length; pi++) {
            ...
            if (_rule.MaterialsLimit && _materialsAll) {
                ...
                var soloMax = _fastGetRecipeQuantity(pair.rd, _materialsMap, preciseChef);
                soloMax = Math.min(soloMax, _fastMaterialLimit(_materialsAll, pair.rd, preciseChef));
    ...
    function _getFullRecipeRanking(chefIndex, recipeIndex, topK) {
        // beam搜索关键路径：调用频率低，放大精确评估范围以保证质量
        return _fastGetRecipeRankingBase(chefIndex, recipeIndex, topK, true, Math.max((topK || 5) * 3, 20));
    }
    ...
    function _estimateMaterialConflictPenalty(recipeData, quantity, chefIndex, chefObj) {
        ...
        for (var m = 0; m < recipeData.materials.length; m++) {
            var mat = recipeData.materials[m];
            myPerUnit[mat.material] = _calMaterialReduceFast(chefObj, mat.material, mat.quantity);
        }
        ...
        for (var ci = 0; ci < _simState[0].length; ci++) {
            for (var ri = 0; ri < 3; ri++) {
                ...
                for (var m2 = 0; m2 < rec.data.materials.length; m2++) {
                    ...
                    var perQty2 = _calMaterialReduceFast(recChef, mat2.material, mat2.quantity);
    ...
    function _climbQuantityRedistribute() {
        ...
        for (var si = 0; si < slots.length; si++) {
            ...
            if (rec.quantity < rec.max) {
                ...
                var canMake = rec.max;
                if (globalRemain) {
                    var matLimit = _fastMaterialLimit(globalRemain, rec.data, ruleState[s.ci].chefObj);
            ...
            for (var reci = 0; reci < 3; reci++) {
                ...
                function _calcSharedMaterialUsage(recObj, chefObj) {
                    ...
                    for (var mm = 0; mm < recObj.data.materials.length; mm++) {
                        ...
                        totalUse += _calMaterialReduceFast(chefObj, matInfo.material, matInfo.quantity) * (recObj.quantity || 0);
                    }
                    return totalUse;
                }
                ...
                for (var oci = 0; oci < 3; oci++) {
                    for (var oreci = 0; oreci < 3; oreci++) {
                        ...
                        for (var oki = 0; oki < otherRk.length; oki++) {
                            ...
                            var newQty = _fastGetRecipeQuantity(lowRd, newRemain, ruleState[ci].chefObj);
                            if (_rule.MaterialsLimit && _materialsAll) {
                                var newGlobalRemain = _calcGlobalRemainMaterials(ci, reci);
                                var matLim = _fastMaterialLimit(newGlobalRemain, lowRd, ruleState[ci].chefObj);
