...
function _installFilterDispatcher() {
    var arr = $.fn.dataTableExt.afnFiltering,
        map = {},
        fallback = [],
        re = /getElementById\(["']([^"']+)["']\)/,
        i, fn, m, id;
    ...
    arr.length = 0;
    arr.push(function(e, a, t, i, l) {
        var tid = e.nTable && e.nTable.id,
            fns = tid && map[tid],
            j;
        if (fns) {
            for (j = 0; j < fns.length; j++) {
                if (!fns[j](e, a, t, i, l)) return !1;
            }
        }
        for (j = 0; j < fallback.length; j++) {
            if (!fallback[j](e, a, t, i, l)) return !1;
        }
        return !0;
    });
}
...
function initTables(e, a) {
    ...
    $('.main-nav a[data-toggle="tab"]').on("shown.bs.tab", function(e) {
        ...
        "true" != $(this).attr("data-init") && updateScrollHeight()) : (e.relatedTarget && reInitFixedHeader($(e.relatedTarget).attr("href")), reInitFixedHeader($(e.target).attr("href")))
    }),
...
function reInitFixedHeader(e){var a={".pane-recipes":["#recipe-table"],".pane-chefs":["#chef-table"],".pane-equips":["#equip-table"],".pane-ambers":["#amber-table"],".pane-decorations":["#decoration-table"],".pane-quest":["#quest-table"],".pane-condiments":["#condiment-table"],".pane-cal":["#cal-recipes-table"]};if(private&&(a[".pane-cal"]=a[".pane-cal"].concat(["#cal-chefs-table","#cal-equips-table","#cal-materials-table"])),e&&a[e])a[e].forEach(function(e){$(e).DataTable().fixedHeader.adjust()});else if(!e)for(var t in a)a[t].forEach(function(e){$(e).DataTable().fixedHeader.adjust()})}
...
function showCalSubPane() {
    ...
    updateScrollHeight())) : reInitFixedHeader(".pane-cal")
}
...
function getCustomRecipesOptions(e, a, t, i) {
    ...
    var g = []
      , k = u.menus;
    // 预计算非目标位置分数
    var _baseQ = getPartialRecipeAdds(h, u), _baseE = getIntentAdds(e, h, i, !1), _baseOther = 0, _baseScores = {};
    for (var _b in h)
        for (var _v in h[_b].recipes)
            if (h[_b].recipes[_v].data && !(_b == a && _v == t)) {
                var _idx = 3 * _b + Number(_v), _M = getRecipeResult(h[_b].chef, h[_b].equip, h[_b].recipes[_v].data, h[_b].recipes[_v].quantity, h[_b].recipes[_v].max, u.materials, u, u.decorationEffect, h[_b].recipes[_v].useCondiment ? h[_b].condiment : null, !0, h[_b].recipes, _baseQ[_idx], _baseE[_idx]);
                h[_b].recipes[_v] = _M, _baseScores[_idx] = _M.totalScore, _baseOther += _M.totalScore;
            }
    for (var y in k) {
        var x = k[y].recipe;
...
$(function() {
    ...
    $("#btn-banquet-remote").click(function() {
        var LAMBDA_URL = "https://dgyzmk7vcybel7smye7k4elgsa0vshlu.lambda-url.ap-northeast-1.on.aws/";
        var ruleId = calCustomRule.id;
        if (!ruleId || ruleId === 0 || typeof ruleId === "string") {
            showAlert("请先选择风云宴赛事", "提示");
            return;
        }

        var $btn = $(this);
        var originalText = $btn.html();
        $btn.prop("disabled", true).html("远程计算中...");
        $("#banquet-progress-bar").css("width", "100%");
        $("#banquet-progress-text").text("等待服务器响应...");
        $("#banquet-progress-wrapper").removeClass("hidden");

        var archive = { msg: JSON.parse(generateExportData()) };
        ...
        $.ajax({
            ...
            data: JSON.stringify({ ruleId: ruleId, archive: archive, excludeMaterials: ($("#chk-cal-recipe-material-exclude").val() || []).map(Number) }),
            timeout: 120000,
            success: function(resp) {
                $btn.prop("disabled", false).html(originalText);
                $("#banquet-progress-wrapper").addClass("hidden");
                if (resp.error) {
                    showAlert(resp.error, "错误");
                    return;
                }
                if (resp.detail) {
                    $("#input-import-run").val(resp.detail);
                    $("#btn-import-run").click();
                }
                var wallMs = Date.now() - startTime;
                showAlert("远程计算完成！得分: " + resp.score + "，计算耗时: " + (resp.timeMs / 1000).toFixed(1) + "秒，总耗时: " + (wallMs / 1000).toFixed(1) + "秒", "成功");
            },
            error: function(xhr) {
                $btn.prop("disabled", false).html(originalText);
                $("#banquet-progress-wrapper").addClass("hidden");
