    ...
    $("select.select-picker-recipe").on("shown.bs.select", function() {
        ...
        function filterRecipes(categoryName) {
            ...
            runeList.forEach(function(runeName) {
                ...
                // 冲突检测：考虑场上已选择的菜谱
                // 1. 先检查场上是否有该贵客的菜谱
                // 2. 如果场上有，则选择框中该贵客的所有菜谱都标记为冲突
                // 3. 如果场上没有，则按原逻辑判断（多个菜谱时有冲突）
                var noConflictRecipes = [];
                var conflictRecipes = [];
                
                // 用于跟踪已处理的菜谱，避免重复添加（双贵客菜谱）
                var processedRecipes = new Set();
                
                Object.keys(recipesByGuest).forEach(function(guestName) {
                    var guestRecipes = recipesByGuest[guestName];
                    
                    // 检查场上是否已有该贵客的菜谱
                    var hasSelectedOnField = false;
                    if (gameData && gameData.guests) {
                        gameData.guests.forEach(function(guest) {
                            if (guest.name === guestName && guest.gifts) {
                                guest.gifts.forEach(function(gift) {
                                    // 检查场上是否有该贵客的任何菜谱
                                    for (var i = 0; i < selectedRecipesOnField.length; i++) {
                                        if (gift.recipe === selectedRecipesOnField[i].recipeName) {
                                            hasSelectedOnField = true;
                                            break;
                                        }
                                    }
                                });
                    ...
                    if (hasSelectedOnField) {
                        // 场上已有该贵客的菜谱，选择框中该贵客的所有菜谱都标记为冲突
                        guestRecipes.forEach(function(recipeInfo) {
                            // 避免重复添加
                            if (!processedRecipes.has(recipeInfo.recipeName)) {
                                conflictRecipes.push(recipeInfo.$li);
                                processedRecipes.add(recipeInfo.recipeName);
                            }
                        });
                    } else if (guestRecipes.length === 1) {
                        // 场上没有，且只有一个菜谱，无冲突
                        // 避免重复添加
                        if (!processedRecipes.has(guestRecipes[0].recipeName)) {
                            noConflictRecipes.push(guestRecipes[0].$li);
                            processedRecipes.add(guestRecipes[0].recipeName);
                        }
                    } else if (guestRecipes.length > 1) {
                        // 场上没有，但有多个菜谱，有冲突
                        // 计算每个菜谱的调整后时间（双贵客菜谱时间除以2）
                        guestRecipes.forEach(function(recipeInfo) {
                            // 计算该菜谱对应的贵客数量
                            var guestCount = 0;
                            if (gameData && gameData.guests) {
                            ...
                            // 调整时间：双贵客菜谱时间除以2
                            recipeInfo.adjustedTime = guestCount >= 2 ? recipeInfo.time / 2 : recipeInfo.time;
                            recipeInfo.guestCount = guestCount;
                        });
                        
                        // 按调整后时间排序
                        guestRecipes.sort(function(a, b) {
                        ...
                        // 其余菜谱都是冲突菜谱
                        for (var i = 1; i < guestRecipes.length; i++) {
                            // 避免重复添加
                            if (!processedRecipes.has(guestRecipes[i].recipeName)) {
                                conflictRecipes.push(guestRecipes[i].$li);
                                processedRecipes.add(guestRecipes[i].recipeName);
                            }
                        }
                    }
                });
                
                // 在“有冲突”列表中排除当前格子自己这道已上场的菜，
                // 避免出现“我正在用松鼠桂鱼，有冲突列表里也有松鼠桂鱼”的情况
                if (currentSelectedRecipeName) {
                    conflictRecipes = conflictRecipes.filter(function($li) {
                        var name = $li.find('.name').text().trim();
                        var keep = (name !== currentSelectedRecipeName);
                        return keep;
                    });
                }
                
                // 标记菜谱的冲突类型
                noConflictRecipes.forEach(function($li) {
                    $li.attr('data-conflict-type', 'none');
                });
                conflictRecipes.forEach(function($li) {
                    $li.attr('data-conflict-type', 'has');
                });
                
                // 更新该符文的冲突统计
                recipesByRune[runeName] = {
                    all: recipes,
                    noConflict: noConflictRecipes,
                    conflict: conflictRecipes
                };
            });
            ...
            // 按符文分组显示（不移动菜谱，只插入分组标题和冲突标签页）
            runeList.forEach(function(runeName) {
                var runeData = recipesByRune[runeName];
                if (!runeData || !runeData.all || runeData.all.length === 0) return;
                
                var recipes = runeData.all;
                var noConflictRecipes = runeData.noConflict || [];
                var conflictRecipes = runeData.conflict || [];
                
                // 检查全局状态，判断该符文是否应该展开
                var isExpanded = runeGroupExpandState[runeName] || false;
                
                // 创建符文分组标题（使用灰色背景和箭头样式，参考规则选择框）
                // 仅作为展示用节点，带 disabled 标记，避免被 bootstrap-select 当成可选项
                var $groupHeader = $('<li class="rune-group-header disabled" data-original-index="-1" style="background: #f5f5f5; padding: 10px 15px; font-weight: bold; color: #337ab7; cursor: pointer; user-select: none; border-bottom: 1px solid #ddd; position: relative;">' +
                    '<div style="display: flex; justify-content: space-between; align-items: center;">' +
                    '<span class="rune-name">' + runeName + ' (' + recipes.length + ')</span>' +
                    '<span class="collapse-icon caret" style="position: absolute; right: 15px; top: 50%; transform: translateY(-50%)' + (isExpanded ? '' : ' rotate(-90deg)') + '; transition: transform 0.2s;"></span>' +
                    '</div>' +
                    '</li>');
                
                // 创建冲突标签页容器（默认隐藏，只有展开时才显示）
                // 同样带 disabled 标记和 data-original-index=-1，仅用于展示
                var $conflictTabs = $('<li class="rune-conflict-tabs disabled" data-original-index="-1" style="background: #e8f4f8; padding: 8px 15px; border-bottom: 1px solid #ddd; display: ' + (isExpanded ? '' : 'none') + ';">' +
                    '<div style="display: flex; gap: 10px;">' +
                    '<button class="conflict-tab conflict-tab-none active" data-conflict="none" style="flex: 1; padding: 8px 12px; border: none; border-radius: 20px; background: #337ab7; color: white; cursor: pointer; font-size: 14px; transition: all 0.3s;">无冲突 (' + noConflictRecipes.length + ')</button>' +
                    '<button class="conflict-tab conflict-tab-has" data-conflict="has" style="flex: 1; padding: 8px 12px; border: none; border-radius: 20px; background: #d0e8f2; color: #337ab7; cursor: pointer; font-size: 14px; transition: all 0.3s;">有冲突 (' + conflictRecipes.length + ')</button>' +
                    '</div>' +
                    '</li>');
                
                // 找到该符文分组的第一个菜谱，在它前面插入分组标题和冲突标签页
                if (recipes.length > 0) {
                    var $firstRecipe = recipes[0];
                    $firstRecipe.before($groupHeader);
                    $groupHeader.after($conflictTabs);
                }
                
                // 收集当前符文分类中所有菜谱的贵客信息，用于检测内部冲突
                var guestToRecipesMap = {};  // 贵客 -> 菜谱列表的映射
                recipes.forEach(function($recipe) {
                    var guestNames = $recipe.attr('data-guest-names');
                    var recipeName = $recipe.find('a .name').text().trim();
                    if (guestNames) {
                        var guestArray = guestNames.split('、');
                ...
                // 找出有冲突的贵客（在当前符文分类中对应多个菜谱的贵客）
                var conflictGuestsInRune = [];
                Object.keys(guestToRecipesMap).forEach(function(guestName) {
                    if (guestToRecipesMap[guestName].length > 1) {
                        conflictGuestsInRune.push(guestName);
                    }
                });
                
                // 找出与场上菜谱有冲突的贵客
                var conflictGuestsWithField = [];
                    ...
                    gameData.guests.forEach(function(guest) {
                        if (guest.gifts) {
                            guest.gifts.forEach(function(gift) {
                                for (var i = 0; i < selectedRecipesOnField.length; i++) {
                                    if (gift.recipe === selectedRecipesOnField[i].recipeName) {
                                        if (conflictGuestsWithField.indexOf(guest.name) === -1) {
                                            conflictGuestsWithField.push(guest.name);
                                        }
                                        break;
                                    }
                                }
                ...
                // 合并两种冲突贵客
                var allConflictGuests = conflictGuestsInRune.concat(conflictGuestsWithField);
                // 去重
                allConflictGuests = allConflictGuests.filter(function(item, index) {
                    return allConflictGuests.indexOf(item) === index;
                });
                
                // 为当前符文分组的菜谱设置显示和位置
                recipes.forEach(function($recipe) {
                    var conflictType = $recipe.attr('data-conflict-type') || 'none';
                    var shouldShow = isExpanded && conflictType === 'none';  // 默认显示无冲突的菜谱
                    ...
                    if (guestNames) {
                        ...
                        if ($name.length > 0) {
                            ...
                            var guestHtml = guestArray.map(function(guestName) {
                                // 检查该贵客是否在冲突列表中
                                var hasConflict = allConflictGuests.indexOf(guestName) !== -1;
                            ...
                            // 修改显示：菜谱名 + 贵客信息在同一行，其他信息换行
                            $name.html(recipeName + ' <span style="font-size: 12px; color: #666;">(' + guestHtml + ')</span>');
                        }
                    }
                    
                    // 设置显示状态并标记
                    $recipe.css({display: shouldShow ? '' : 'none'})
                        .addClass('rune-group-item')
                        .attr('data-rune-group', runeName);  // 标记菜谱属于哪个符文分组
                });
                
                // 绑定冲突标签页点击事件
                $conflictTabs.find('.conflict-tab').off('click').on('click', function(e) {
                    e.stopPropagation();
                    var $tab = $(this);
                    var conflictType = $tab.attr('data-conflict');
                    
                    // 切换标签页激活状态
                    $tab.addClass('active').siblings().removeClass('active');
                    
                    // 更新按钮样式
                    $conflictTabs.find('.conflict-tab').each(function() {
                        var $btn = $(this);
                        if ($btn.hasClass('active')) {
                            $btn.css({background: '#337ab7', color: 'white'});
                        } else {
                            $btn.css({background: '#d0e8f2', color: '#337ab7'});
                        }
                    });
                    
                    // 检查是否有搜索条件
                    var $searchInput = $dropdown.find('.bs-searchbox input');
                    var searchText = $searchInput.length > 0 ? $searchInput.val().trim().toLowerCase() : '';
                    
                    // 根据选择的标签页显示/隐藏菜谱
                    recipes.forEach(function($recipe) {
                        var recipeConflictType = $recipe.attr('data-conflict-type') || 'none';
                        
                        if (recipeConflictType === conflictType) {
                            // 如果有搜索条件，还需要检查菜谱名是否匹配
                            if (searchText) {
                                var recipeName = $recipe.find('a .name').text().toLowerCase();
                ...
                // 绑定分组标题折叠/展开事件
                $groupHeader.off('click').on('click', function(e) {
                    e.stopPropagation();
                    var $header = $(this);
                    ...
                    // 检查是否有搜索文本
                    var $searchInput = $dropdown.find('.bs-searchbox input');
                    var searchText = $searchInput.length > 0 ? $searchInput.val().trim() : '';
                    
                    if (isExpanded) {
                        // 当前是展开状态，点击后折叠
                        $conflictTabs.slideUp(200);
                        recipes.forEach(function($recipe) {
                            $recipe.slideUp(200);
                        });
                        $icon.css('transform', 'translateY(-50%) rotate(-90deg)');
                        ...
                        // 当前是折叠状态，点击后展开
                        $conflictTabs.slideDown(200);
                        
                        // 根据当前激活的冲突标签页和搜索条件显示对应的菜谱
                        var activeConflictType = $conflictTabs.find('.conflict-tab.active').attr('data-conflict') || 'none';
                        ...
                        recipes.forEach(function($recipe) {
                            ...
                            // 首先检查冲突类型是否匹配
                            if (recipeConflictType === activeConflictType) {
                                // 如果有搜索条件，还需要检查菜谱名是否匹配
                                if (searchText) {
                                    var recipeName = $recipe.find('a .name').text().toLowerCase();
                                    if (recipeName.indexOf(searchTextLower) >= 0) {
                                        $recipe.slideDown(200);
                                    } else {
                                        // 不匹配搜索条件，保持隐藏
                                        $recipe.css('display', 'none');
                                    }
                ...
                if (currentCategory) {
                    ...
                    if (searchText) {
                        ...
                        $innerMenu.find('li').each(function() {
                            ...
                            // 跳过分组标题和冲突标签
                            if ($li.hasClass('rune-group-header') || $li.hasClass('rune-conflict-tabs')) {
                                return; // continue
                            }
                            ...
                            // 判断是否匹配搜索文本
                            if (recipeName.indexOf(searchText) >= 0) {
                                // 匹配：显示（使用css而不是show，避免与slideDown冲突）
                                $li.css('display', '');
                            } else {
                                // 不匹配：隐藏
                                $li.css('display', 'none');
                        ...
                        // 更新符文分组标题的计数和冲突标签的计数
                        $innerMenu.find('li.rune-group-header').each(function() {
                            var $header = $(this);
                            var runeName = $header.find('.rune-name').text().split(' (')[0];
                            
                            // 统计该符文下可见的菜谱数量（按冲突类型分类）
                            var visibleNoConflictCount = 0;
                            var visibleHasConflictCount = 0;
                            var $nextLi = $header.next();
                            
                            // 获取冲突标签元素
                            var $conflictTabs = null;
                            if ($nextLi.hasClass('rune-conflict-tabs')) {
                                $conflictTabs = $nextLi;
                                $nextLi = $nextLi.next();
                            }
                            
                            // 遍历该符文的所有菜谱
                            while ($nextLi.length && !$nextLi.hasClass('rune-group-header')) {
                                if (!$nextLi.hasClass('rune-conflict-tabs') && 
                                    !$nextLi.hasClass('divider') && 
                                    !$nextLi.hasClass('disabled') &&
                                    $nextLi.css('display') !== 'none') {
                                    
                                    // 根据冲突类型统计
                                    var conflictType = $nextLi.attr('data-conflict-type') || 'none';
                                    if (conflictType === 'none') {
                                        visibleNoConflictCount++;
                                    } else if (conflictType === 'has') {
                                        visibleHasConflictCount++;
                                    }
                                }
                                $nextLi = $nextLi.next();
                            }
                            
                            var totalVisibleCount = visibleNoConflictCount + visibleHasConflictCount;
                            
                            // 更新标题文本
                            $header.find('.rune-name').text(runeName + ' (' + totalVisibleCount + ')');
                            
                            // 更新冲突标签的计数
                            if ($conflictTabs && $conflictTabs.length > 0) {
                                $conflictTabs.find('.conflict-tab-none').text('无冲突 (' + visibleNoConflictCount + ')');
                                $conflictTabs.find('.conflict-tab-has').text('有冲突 (' + visibleHasConflictCount + ')');
                            }
                            
                            // 如果该符文下没有可见的菜谱，隐藏分组标题和冲突标签
                            if (totalVisibleCount === 0) {
                                $header.css('display', 'none');
                                if ($conflictTabs && $conflictTabs.length > 0) {
                                    $conflictTabs.css('display', 'none');
                                }
                            } else {
                                $header.css('display', '');
                                // 搜索模式下：如果符文有匹配的菜谱，临时展开并显示冲突标签（不保存展开状态）
                                if ($conflictTabs && $conflictTabs.length > 0) {
                                    $conflictTabs.css('display', '');
                                    
                                    // 更新箭头方向为展开状态（仅视觉效果）
                                    $header.find('.collapse-icon').css('transform', 'translateY(-50%) rotate(0deg)');
                                    
                                    // 注意：不更新 runeGroupExpandState，这样清空搜索后会恢复原状态
                                    
                                    // 确保当前激活的冲突类型的菜谱可见
                                    var activeConflictType = $conflictTabs.find('.conflict-tab.active').attr('data-conflict') || 'none';
                                    var $nextRecipe = $conflictTabs.next();
                                    while ($nextRecipe.length && !$nextRecipe.hasClass('rune-group-header')) {
                                        if ($nextRecipe.hasClass('rune-group-item')) {
                                            var recipeConflictType = $nextRecipe.attr('data-conflict-type') || 'none';
                                            var recipeName = $nextRecipe.find('a .name').text().toLowerCase();
                                            
                                            // 如果菜谱匹配搜索且属于当前激活的冲突类型，显示它
                                            if (recipeName.indexOf(searchText) >= 0 && recipeConflictType === activeConflictType) {
                                                $nextRecipe.css('display', '');
                                            } else if (recipeConflictType !== activeConflictType) {
                                                // 不属于当前激活的冲突类型，隐藏
                                                $nextRecipe.css('display', 'none');
                                            }
                                        }
                        ...
                        $innerMenu.find('li').each(function() {
                            ...
                            // 分组标题始终显示
                            if ($li.hasClass('rune-group-header')) {
                                $li.css('display', '');
                                return; // continue
                            }
                            
                            // 冲突标签根据展开状态决定是否显示
                            if ($li.hasClass('rune-conflict-tabs')) {
                                // 找到对应的符文分组标题
                                var $header = $li.prev();
                                if ($header.hasClass('rune-group-header')) {
                                    var runeName = $header.find('.rune-name').text().split(' (')[0];
                                    var isExpanded = runeGroupExpandState[runeName] || false;
                            ...
                            // 菜谱根据冲突类型和展开状态决定是否显示
                            if ($li.hasClass('rune-group-item')) {
                                var runeName = $li.attr('data-rune-group');
                                var isExpanded = runeGroupExpandState[runeName] || false;
                                var conflictType = $li.attr('data-conflict-type') || 'none';
                                
                                // 找到该符文的冲突标签，获取当前激活的冲突类型
                                var $header = $innerMenu.find('li.rune-group-header').filter(function() {
                                    return $(this).find('.rune-name').text().indexOf(runeName) === 0;
                                });
                                var $conflictTabs = $header.next();
                                if ($conflictTabs.hasClass('rune-conflict-tabs')) {
                                    var activeConflictType = $conflictTabs.find('.conflict-tab.active').attr('data-conflict') || 'none';
                                    
                                    // 根据展开状态和冲突类型决定是否显示
                                    if (isExpanded && conflictType === activeConflictType) {
                                        $li.css('display', '');
                                    } else {
                                        $li.css('display', 'none');
                                    }
                        ...
                        $innerMenu.find('li.rune-group-header').each(function() {
                            ...
                            // 统计该符文下的总菜谱数量（按冲突类型分类）
                            var totalCount = 0;
                            var noConflictCount = 0;
                            var hasConflictCount = 0;
                            var $nextLi = $header.next();
                            ...
                            // 遍历该符文的所有菜谱
                            while ($nextLi.length && !$nextLi.hasClass('rune-group-header')) {
                                // 只统计真正的菜谱项（有 rune-group-item 类）
                                if ($nextLi.hasClass('rune-group-item')) {
                                    totalCount++;
                            ...
                            // 更新标题文本
                            $header.find('.rune-name').text(runeName + ' (' + totalCount + ')');
                            
                            // 恢复冲突标签的计数
                            if ($conflictTabs && $conflictTabs.length > 0) {
                                $conflictTabs.find('.conflict-tab-none').text('无冲突 (' + noConflictCount + ')');
                                $conflictTabs.find('.conflict-tab-has').text('有冲突 (' + hasConflictCount + ')');
                            }
                            ...
                            if (isExpanded) {
                                ...
                                // 收起状态：箭头向右
                                $header.find('.collapse-icon').css('transform', 'translateY(-50%) rotate(-90deg)');
                            }
                        });
                    }
                }
