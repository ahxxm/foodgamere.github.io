...
function setCalPartialUltimateOptions(e) {
    // 计算器页面专用：显示所有具有Partial或Next条件修炼技能的厨师，不进行类型区分
    var allPartialChefs = [];
    ...
    for (var t in e) {
        ...
        if (hasPartialOrNext) {
            var chefData = {};
            chefData.chef = e[t];
            chefData.disp = e[t].ultimateSkillDisp;
            allPartialChefs.push(chefData);
        }
    }
    
    // 按厨师名称排序
    allPartialChefs.sort(function(a, b) {
        return a.chef.name.localeCompare(b.chef.name);
    });
    
    // 添加到计算器选择框
    for (var t in allPartialChefs) {
        var s = "<option value='" + allPartialChefs[t].chef.chefId + "' data-subtext='" + allPartialChefs[t].disp.replace(/<br>/g, " ") + "'>" + allPartialChefs[t].chef.name + "</option>";
        $("#chk-cal-partial-ultimate").append(s);
    }
}
